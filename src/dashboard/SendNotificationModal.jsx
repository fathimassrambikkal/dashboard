...
const SendNotificationModal = ({
  ...
  const isCustomerSelected = (c) =>
    selectedCustomers.some(x => x.id === c.id);

  const isProductSelected = (p) =>
  ...
  return (
    <div
      className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 z-50 overflow-x-hidden"
    >
      <div
        className="bg-white/90 backdrop-blur-lg rounded-xl sm:rounded-2xl w-full max-w-6xl max-h-[95vh] sm:max-h-[90vh]
        overflow-y-auto border border-gray-200/60 shadow-[0_20px_60px_rgba(0,0,0,0.2)] mx-2 sm:mx-4 
        overflow-x-hidden min-w-0"
      >

        {/* HEADER */}
        <div className="p-4 sm:p-6 border-b border-gray-200/60 relative min-w-0">
          <button
          ...
          {/* STEPPER */}
          <div className="flex justify-center items-center gap-2 sm:gap-4 mb-4 sm:mb-6 min-w-0 overflow-x-hidden">
            {[1, 2, 3].map((step) => (
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2 sm:mb-4 text-center">
            Send Notification
          </h2>
        </div>
        ...
        {notificationStep === 1 && (
          <div className="p-4 sm:p-6 min-w-0 overflow-x-hidden">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-2 mb-4 min-w-0">
              ...
              >
                {selectedCustomers.length === loyalCustomers.length
                  ? 'Deselect All'
                  : 'Select All'}
              </button>
            </div>

            <div className="space-y-2 max-h-[50vh] sm:max-h-[40vh] overflow-y-auto min-w-0">
              {loyalCustomers.map(customer => (
                <div
                  key={customer.id}
                  onClick={() => handleCustomerSelect(customer)}
                  className={`flex items-center justify-between p-3 sm:p-4 rounded-xl border-2 cursor-pointer transition-all min-w-0
                  ${
        ...
        {notificationStep === 2 && (
          <div className="p-4 sm:p-6 min-w-0 overflow-x-hidden">
            ...
            <div className="space-y-3 max-h-[50vh] sm:max-h-[40vh] overflow-y-auto min-w-0">
              {[
                ...
                <div
                  ...
                  className={`p-4 border-2 rounded-xl cursor-pointer transition-all min-w-0
                  ${
        ...
        {notificationStep === 3 && (
          <div className="p-4 sm:p-6 min-w-0 overflow-x-hidden">
            ...
            {filteredProducts.length === 0 ? (
              ...
              <>
                {/* Product Grid */}
                <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[45vh] overflow-y-auto min-w-0">
                  {filteredProducts.map(product => (
                    <div
                      key={product.id}
                      onClick={() => handleProductSelect(product)}
                      className={`p-4 rounded-2xl bg-white/80 border transition-all cursor-pointer min-w-0
                      ${
                ...
                {/* Buttons */}
                <div className="flex flex-col sm:flex-row gap-2 mt-6 min-w-0">
                  <button
                    onClick={() => setNotificationStep(2)}
                    className="flex-1 bg-gray-500 text-white py-2 rounded-lg hover:bg-gray-600"
                  >
